...
class AtmosphereTemplate:
    ...
    def to_dict(self) -> dict:
        return {'name': self.name, 'description': self.description, 'wavelengths_um': self.wavelengths_um.tolist(), 'transit_depth_ppm': self.transit_depth_ppm.tolist(), 'parameters': self.parameters}

    @classmethod
    def from_dict(cls, d: dict) -> 'AtmosphereTemplate':
        return cls(name=d['name'], description=d['description'], wavelengths_um=np.array(d['wavelengths_um']), transit_depth_ppm=np.array(d['transit_depth_ppm']), parameters=d['parameters'])

    @property
    def dominant_species(self) -> str:
        return {'earth_like': 'O2 + H2O + CO2 + O3', 'high_co2': 'CO2 (dominant)', 'reduced_o2_high_ch4': 'CH4 + H2O + CO2'}.get(self.name, self.name)
...
def build_earth_like_template(wavelengths: np.ndarray, cloud_fraction: float=0.5, o2_ch4_ratio: float=1.0, scale_height_km: float=8.5, planet_radius_re: float=1.0, star_radius_rs: float=0.2) -> AtmosphereTemplate:
    ...
    return AtmosphereTemplate(name='earth_like', description='Earth-analog: O2 + H2O + CO2 + O3 + trace CH4 (biosignature-rich)', wavelengths_um=wavelengths, transit_depth_ppm=np.maximum(depth, 0.0), parameters={'cloud_fraction': float(cloud_fraction), 'o2_ch4_ratio': float(o2_ch4_ratio), 'scale_height_km': float(scale_height_km), 'planet_radius_re': float(planet_radius_re), 'star_radius_rs': float(star_radius_rs), 'base_depth_ppm': float(base)})

def build_high_co2_template(wavelengths: np.ndarray, cloud_fraction: float=0.3, o2_ch4_ratio: float=0.01, scale_height_km: float=7.0, planet_radius_re: float=1.0, star_radius_rs: float=0.2) -> AtmosphereTemplate:
    base = _base_depth_ppm(planet_radius_re, star_radius_rs)
    amp = _atm_amplitude_ppm(scale_height_km, planet_radius_re, star_radius_rs)
    f_clear = 1.0 - cloud_fraction
    depth = np.full_like(wavelengths, base)
    co2 = [(1.05, 0.04, 0.3), (1.21, 0.045, 0.35), (1.43, 0.055, 0.55), (1.6, 0.07, 0.8), (2.01, 0.1, 1.2), (2.68, 0.2, 1.5), (4.3, 0.45, 2.0)]
    for center, width, strength in co2:
        depth += f_clear * amp * strength * _gaussian(wavelengths, center, width)
    ...
    return AtmosphereTemplate(name='high_co2', description='Venus-analog: CO2-dominated, pressure-broadened, no biosignatures', wavelengths_um=wavelengths, transit_depth_ppm=np.maximum(depth, 0.0), parameters={'cloud_fraction': float(cloud_fraction), 'o2_ch4_ratio': float(o2_ch4_ratio), 'scale_height_km': float(scale_height_km), 'planet_radius_re': float(planet_radius_re), 'star_radius_rs': float(star_radius_rs), 'base_depth_ppm': float(base)})

def build_reduced_o2_high_ch4_template(wavelengths: np.ndarray, cloud_fraction: float=0.4, o2_ch4_ratio: float=0.001, scale_height_km: float=9.0, planet_radius_re: float=1.0, star_radius_rs: float=0.2) -> AtmosphereTemplate:
    base = _base_depth_ppm(planet_radius_re, star_radius_rs)
    amp = _atm_amplitude_ppm(scale_height_km, planet_radius_re, star_radius_rs)
    f_clear = 1.0 - cloud_fraction
    ...
    return AtmosphereTemplate(name='reduced_o2_high_ch4', description='Anoxic biosphere: high CH4 + H2O + CO2, negligible O2 (Archean analog)', wavelengths_um=wavelengths, transit_depth_ppm=np.maximum(depth, 0.0), parameters={'cloud_fraction': float(cloud_fraction), 'o2_ch4_ratio': float(o2_ch4_ratio), 'scale_height_km': float(scale_height_km), 'planet_radius_re': float(planet_radius_re), 'star_radius_rs': float(star_radius_rs), 'base_depth_ppm': float(base)})

def build_abiotic_o2_template(wavelengths: np.ndarray, cloud_fraction: float=0.3, o2_ch4_ratio: float=1000.0, scale_height_km: float=7.0, planet_radius_re: float=1.0, star_radius_rs: float=0.2) -> AtmosphereTemplate:
    base = _base_depth_ppm(planet_radius_re, star_radius_rs)
    amp = _atm_amplitude_ppm(scale_height_km, planet_radius_re, star_radius_rs)
    f_clear = 1.0 - cloud_fraction
    depth = np.full_like(wavelengths, base)
    ...
    return AtmosphereTemplate(name='abiotic_o2', description='Abiotic O2 false positive: CO2-dominated, desiccated, photolytic O2/O3, CH4-free (Krissansen-Totton et al. 2021)', wavelengths_um=wavelengths, transit_depth_ppm=np.maximum(depth, 0.0), parameters={'cloud_fraction': float(cloud_fraction), 'o2_ch4_ratio': float(o2_ch4_ratio), 'scale_height_km': float(scale_height_km), 'planet_radius_re': float(planet_radius_re), 'star_radius_rs': float(star_radius_rs), 'base_depth_ppm': float(base), 'reference': 'Krissansen-Totton et al. 2021, AGU Advances, 2, e2020AV000294'})

def build_hycean_template(wavelengths: np.ndarray, cloud_fraction: float=0.2, o2_ch4_ratio: float=0.002, scale_height_km: float=12.0, planet_radius_re: float=2.3, star_radius_rs: float=0.4, dms_strength: float=0.08, dmds_strength: float=0.0) -> AtmosphereTemplate:
    base = _base_depth_ppm(planet_radius_re, star_radius_rs)
...
class TemplateGrid:
    ...
    def build_grid(self, template_names: Optional[List[str]]=None, cloud_fractions: List[float]=[0.0, 0.3, 0.6, 0.9], scale_heights_km: List[float]=[6.0, 8.5, 11.0], o2_ch4_ratios: Optional[Dict[str, List[float]]]=None, planet_radius_re: float=1.0, star_radius_rs: float=0.2) -> None:
        if template_names is None:
            template_names = list(self.BUILDERS.keys())
        if o2_ch4_ratios is None:
            o2_ch4_ratios = self.DEFAULT_O2_CH4
        self._meta = {'cloud_fractions': cloud_fractions, 'scale_heights_km': scale_heights_km, 'o2_ch4_ratios': o2_ch4_ratios, 'planet_radius_re': planet_radius_re, 'star_radius_rs': star_radius_rs}
        total = 0
    ...
    def get_template(self, template_name: str, cloud_fraction: float=0.5, o2_ch4_ratio: float=1.0, scale_height_km: float=8.5) -> AtmosphereTemplate:
        if template_name not in self.templates or not self.templates[template_name]:
            raise KeyError(f"Template '{template_name}' not in grid. Available: {list(self.templates.keys())}. Call build_grid() first.")
        candidates = self.templates[template_name]

        def _dist(t: AtmosphereTemplate) -> float:
            p = t.parameters
            dc = abs(p['cloud_fraction'] - cloud_fraction)
            dr = abs(np.log10(max(p['o2_ch4_ratio'], 1e-07)) - np.log10(max(o2_ch4_ratio, 1e-07))) * 0.5
    ...
    def save(self, path: str) -> None:
        ...
        payload = {'wavelengths_um': self.wavelengths.tolist(), 'meta': self._meta, 'templates': {name: [t.to_dict() for t in lst] for name, lst in self.templates.items()}}
    ...
    def load(cls, path: str) -> 'TemplateGrid':
        ...
        grid.templates = {name: [AtmosphereTemplate.from_dict(d) for d in lst] for name, lst in data['templates'].items()}
        total = sum((len(v) for v in grid.templates.values()))
        print(f'TemplateGrid loaded: {total} templates from {path}')
        return grid

def get_default_templates(star_radius_rs: float=0.2, planet_radius_re: float=1.0) -> Dict[str, AtmosphereTemplate]:
    wl = default_wavelength_grid()
    return {'earth_like': build_earth_like_template(wl, cloud_fraction=0.5, o2_ch4_ratio=1.0, scale_height_km=8.5, planet_radius_re=planet_radius_re, star_radius_rs=star_radius_rs), 'high_co2': build_high_co2_template(wl, cloud_fraction=0.3, o2_ch4_ratio=0.01, scale_height_km=7.0, planet_radius_re=planet_radius_re, star_radius_rs=star_radius_rs), 'reduced_o2_high_ch4': build_reduced_o2_high_ch4_template(wl, cloud_fraction=0.4, o2_ch4_ratio=0.001, scale_height_km=9.0, planet_radius_re=planet_radius_re, star_radius_rs=star_radius_rs), 'hycean': build_hycean_template(wl, cloud_fraction=0.2, o2_ch4_ratio=0.002, scale_height_km=12.0, planet_radius_re=max(planet_radius_re * 2.0, 2.3), star_radius_rs=max(star_radius_rs * 2.0, 0.4))}
