...
false_positive.py
-----------------
Injection/recovery test framework for biosignature detection.

What this file does:
  This module answers the question referees always ask first:
  "What is your false positive rate?"

  It does injection/recovery testing: inject a known atmosphere signal
  into simulated noise, run the full retrieval pipeline, and measure:
    - True positive rate: "we said earth-like, it was earth-like"
    - False positive rate: "we said earth-like, it was high-CO2"
    - Confusion matrix: full N×N classification accuracy table
    - Detection completeness: what fraction of real biosignatures do we detect?

  Run this across the full parameter space (cloud fraction × scale height ×
  distance × n_transits) to build the false positive rate table that goes
  in your paper.

Where to put this file:
  → biosignatures_project/src/false_positive.py
...
Usage:
    ...
    runner = InjectionRecoveryRunner(grid, sim, retrieval)
    table = runner.run(planet, n_trials=200)
    table.print_summary()
"""
