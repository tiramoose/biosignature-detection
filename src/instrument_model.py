...
class InstrumentConfig:
    ...
    n_pixels_per_resolution_element: int
    pixel_scale_arcsec: float
    detector_gain: float
    saturation_electrons: float
    stellar_noise_floor_ppm: float
    notes: str = ""
...
class InstrumentModel:
    ...
    def stellar_sed_calibration(self, wavelengths_um: np.ndarray) -> np.ndarray:
        ...
        with open("data/phoenix/m_dwarf_sed_ratios.csv") as f:
            for line in f:
                ...
                phoenix_wl.append(float(a))
                phoenix_ratio.append(float(b))

        phoenix_wl = np.array(phoenix_wl)
        phoenix_ratio = np.array(phoenix_ratio)
        return np.interp(
            wavelengths_um,
            phoenix_wl,
            phoenix_ratio,
            left=phoenix_ratio[0],
            right=phoenix_ratio[-1],
        )
    ...
    def stellar_photon_rate(
        ...
        j_zp_jy = 1600.0
        F_j_jy = j_zp_jy * 10.0 ** (-star_magnitude_j / 2.5)

        # Convert Jy → erg/s/cm²/Hz at 1.25 μm
        F_j_cgs = F_j_jy * 1e-23

        # Convert to per-μm using dν/dλ = c/λ²
        c_um_s = 3e14
        F_j_per_um = F_j_cgs * c_um_s / (1.25 ** 2)

        # Scale full SED to this J-band flux
        j_idx = np.argmin(np.abs(wavelengths_um - 1.25))
        F_lambda = B_lambda / B_lambda[j_idx] * F_j_per_um

        # --- Photon energy at each wavelength [erg] ---
        E_photon_erg = (6.626e-27 * 3e10) / (wl_m * 100.0)

        # --- Photon rate per unit area per μm ---
        photon_flux_per_um = F_lambda / E_photon_erg

        # --- Integrate over spectral bin width ---
