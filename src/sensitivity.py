...
class SensitivityRunner:
    ...
    # Noise-floor sweep grid, ppm. 20 ppm is the old pre-launch placeholder
    # (Deming+2009, Greene+2016); post-launch data (Rustamkulov+2022 lab
    # analysis -> 3-sigma limit of 14ppm; Rustamkulov+2023 on-orbit WASP-39b
    # PRISM -> "near photon limit") supports 5-10 ppm for a quiet host star.
    # See instrument_model.load_jwst_nirspec() docstring for full citations
    # and the caveat that active M dwarfs (this pipeline's dominant HZ host
    # type) are not covered by these bright-quiet-star results.
    NOISE_FLOORS = [5.0, 10.0, 14.0, 20.0, 30.0, 50.0]
    NOISE_FLOOR_LITERATURE_DEFAULT = 10.0  # matches instrument_model.load_jwst_nirspec() default
