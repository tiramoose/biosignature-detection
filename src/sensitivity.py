...
sensitivity.py
--------------
Systematic sensitivity analysis and bias check for the biosignature pipeline.

What this file does:
  Answers the referee question: "How do your results change when your
  assumptions are wrong?" Varies four key parameters one at a time
  (cloud fraction, stellar noise floor, scale height, transit count)
  and measures the change in detection probability. Also runs a full
  bias check — injecting signals at every grid point and verifying the
  pipeline recovers the correct rate, not just at the fiducial.

  Produces the data behind the sensitivity heatmaps in the paper:
    Figure: detection probability vs. (cloud fraction × distance)
    Figure: detection probability vs. (noise floor × n_transits)
    Table:  bias check — recovery rate at each parameter combination

Where to put this file:
  → biosignatures_project/src/sensitivity.py
...
Usage:
    ...
    results = runner.run_all(planet)
    runner.save(results, 'results/sensitivity/')
"""
